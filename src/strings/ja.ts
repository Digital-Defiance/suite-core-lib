...
export const japaneseStrings: Record<SuiteCoreStringKeyValue, string> = {
  ...
  [SuiteCoreStringKey.Error_UnexpectedErrorInAuthenticateCrypto]:
    'authenticateCryptoで予期しないエラーが発生しました',
  ...
  [SuiteCoreStringKey.Admin_ServingRoute]: 'ルートを提供しています',
  ...
  [SuiteCoreStringKey.Admin_DatabaseInitializedWithOptionsHashTemplate]:
    'オプションハッシュでデータベースが初期化されました: {hash}',
  [SuiteCoreStringKey.Admin_DatabaseAlreadyInitialized]:
    'データベースは既に初期化されています',
  [SuiteCoreStringKey.Admin_DatabaseDropped]:
    'データベースコレクションが削除されました',
  [SuiteCoreStringKey.Admin_DevelopmentDatabase]: '開発データベース',
  [SuiteCoreStringKey.Admin_DroppingDatabase]: 'データベースを削除しています',
  ...
  [SuiteCoreStringKey.Error_EmailSentTooRecentlyTemplate]:
    'メールトークンの送信が頻繁すぎます。${timeRemaining}秒後に再試行してください。',
  ...
  [SuiteCoreStringKey.Login_PasswordAuthInfo]:
    'パスワードは、このブラウザにのみ保存されているリカバリーフレーズを復号化します。新しいデバイスでサインインするには、リカバリーフレーズを直接使用する必要があります。',
  ...
  [SuiteCoreStringKey.Settings_EmailHelper]:
    'メールアドレスを変更すると、再確認が必要になります。新しいアドレスに確認メールを送信します。変更を保存した後、受信トレイを確認し、メールの指示に従って新しいアドレスを確認してください。',
